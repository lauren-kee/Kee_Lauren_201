...
```{r}
library(glue)
bears_spread <- bears - opponent
which.max(bears_spread)
print(glue("The largest win spread is {bears[5]-opponent[5]}, when the Bears scored {bears[5]} points and the Opponent scored {opponent[5]} points."))
```
...
```{r}
difference <- apply(game_score, 1, function(x) if (x[1] > x[2]) print("win") else print("loss"))
```
...
```{r}
win <- apply(game_score, 1, function(x) x[1] > x[2])
win
```

```{r}
# version two for my own reference
...
```{r}
losses <- 0
for(i in game_score[, 3]){
    if (i==0) losses <- losses + 1
    if (i>0) {
        print(losses) 
        break
    }
...
```{r}
area_rectangle <- function(l = 48, w = 48) {
    square_inches <- l*w
    square_feet <- (l/12)*(w/12)
    return(c(square_inches, square_feet))
}

output <- area_rectangle(102, 60)

print(glue("The area in square inches is {output[1]}. 
     The area in square feet is {output[2]}."))
```
...
```{r}
most_letters <- function(string_vec) {
    chars <- sapply(string_vec, function(x) nchar(x)) 
    return(string_vec[which(chars == max(chars))])
}
...
```{r}
check_vec <- function(val, vec){
    return(any(vec==val))
}

output <- check_vec(value, check)

if (output == TRUE) print(glue("{value} is in the vector.")) else print(glue("{value} is not in the vector."))
```

```{r}
#version two for my own reference
#if (val %in% vec) return(TRUE)
#else return(FALSE)
```
...
```{r}
x
odd_even <- function(mat) {
    for (i in 1: nrow(mat)){
      for (j in 1: ncol(mat)) {
          if (mat[i, j] %% 2 == 0) mat[i, j] <- 1 else mat[i, j] <- 0
      }
    }
    return(mat)
}
